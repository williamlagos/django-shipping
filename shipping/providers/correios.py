...
class Encomenda:
    ...
    def adicionar_status(self, status):
        ...
        self.status.sort(key=lambda x: x.data)
...
class CorreiosWebsiteScraper:
    ...
    def get_encomenda_info(self, numero):
        request = urllib.request.urlopen(f"{self.url}{numero}")
        html = request.read()
        request.close()
        if html:
        ...
        for tr in soup.table:
            if count > 4 and str(tr).strip() != "":
                if re.match(r"\d{2}\/\d{2}\/\d{4} \d{2}:\d{2}", tr.contents[0].string):
                    ...
                    status[-1].detalhes = str(tr.contents[0].string)
            count += 1
...
class CorreiosCode:
    ...
    def _parse_linha_tabela(self, tr):
        values = [cell.get_text() for cell in tr.findAll("td")]
    ...
    def _parse_tabela(self, html):
        ...
        linhas = soup.findAll(
            "tr",
            attrs={"onclick": re.compile(r"javascript:detalharCep\('\d+','\d+'\);")},
        )
        return [self._parse_linha_tabela(linha) for linha in linhas]

    def _parse_faixa(self, html):
        decoded = html.decode("cp1252")
        if "não está cadastrada" in decoded:
            return None
        ceps = re.findall(r"\d{5}-\d{3}", decoded)
        if len(ceps) in (4, 6):  # uf (+ uf) + cidade com range
            return tuple(ceps[-2:])
    ...
    def detalhe(self, posicao=0):
        ...
        handle = self._url_open(
            "detalheCEPAction.do",
            {"Metodo": "detalhe", "TipoCep": 2, "Posicao": posicao + 1, "CEP": ""},
        )
        html = handle.read()
    ...
    def _rvirtualurl(self, cep):
        url = (
            f"http://cep.republicavirtual.com.br/web_cep.php?formato=xml&cep={str(cep)}"
        )
    ...
    def consulta(
        self, cep, correios=False, uf=None, localidade=None, tipo=None, numero=None
    ):
...
class CorreiosShippingService:
    ...
    def _validate(self):
        """Valida as medidas (apenas os minimos) para calculo"""
        peso_minimo = 0.3
        ...
        if self.formato == "PACOTE" and self.largura < 11 and self.comprimento < 25:
            self.largura = 11
    ...
    def _request(self):
        """Realiza a requisicao ao webservice e recupera o retorno"""
        url = f"{self.URL}?{urllib.parse.urlencode(self.hash)}"
        self.response = urllib.request.urlopen(url).read()

    def _parse(self):
        """
        Processa o xml retornado pelo webservice utilizando o BeautifulSoup
        ...
        xmltree = BeautifulSoup(self.response, "xml")

        for result in xmltree.find_all("cServico"):
            servico_id = result.find("Codigo").string
            prazo = result.find("PrazoEntrega").string
            valor = result.find("Valor").string
            erro = result.find("Erro").string
            matching = [k for k in SERVICOS if SERVICOS[k] == servico_id]
            if not matching:
                continue
            servico = matching[0]

            if erro != "0":
                msgerro = result.find("MsgErro").string
