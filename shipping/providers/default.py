...
def fretefacil_shipping_handler(request, form, order=None):
    if request.session.get("free_shipping"):
        return
    settings.use_editable()
    if form is not None:
        user_postcode = form.cleaned_data["shipping_detail_postcode"]
    else:
        user_postcode = settings.STORE_POSTCODE
    ...
    if cart.has_items():
        for product in cart:
            ...
            if len(properties) > 0:
                props = properties[0]
                deliverable = shippingservice.create_deliverable(
...
def correios_create_deliverable(
    obj, service, store_postcode, user_postcode, width, height, length, weight
):
...
def sedex_shipping_handler(request, form, order=None):
    ...
    if cart.has_items():
        for product in cart:
            ...
            if len(properties) > 0:
                props = properties[0]
                deliverable = correios_create_deliverable(
                    shippingservice,
                    "SEDEX",
                    settings.STORE_POSTCODE,
                    user_postcode,
...
def shipping_payment_handler(request, order_form, order):
    ...
    order.billing_detail_street = "{} {} {}".format(
        shipping_data["tipo_logradouro"],
